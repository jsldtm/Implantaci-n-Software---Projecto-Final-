...
export const productCatalog: Record<number, ProductCatalogItem> = {
    // Technology category
    1: {
        id: 1,
        name: "Cable USB-B",
...
export const getAllProducts = (): ProductCatalogItem[] => {
  return Object.values(productCatalog);
};

// Helper function to get product by ID
export const getProductById = (id: number): ProductCatalogItem | undefined => {
  return productCatalog[id];
};

// Helper function to get products by category
export const getProductsByCategory = (category: string): ProductCatalogItem[] => {
  return Object.values(productCatalog).filter(product => product.category === category);
};

// Get all unique categories
export const getCategories = (): string[] => {
  const categories = new Set(Object.values(productCatalog).map(product => product.category));
